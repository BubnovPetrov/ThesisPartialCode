...
%% Data
% For now, only consider square domain and square subdomain

L=10; % subdomain side length
ds=1; % KLE discretization spacing
lc1=6;lc2=6; % correlation lengths (with respect to subdomain)
np=9; % number of domain partition (number of subdomains each dimension)
tens=1; % tensorizable corr fcn?
corr_model{1}='gauss';
corr_model{2}='gauss';
dist_norm='L2'; 
err_kl=5e-2; % KLE accuracy of representation
...
%% Prolongation Matrix Generation
tic;
...
%% Random Field Sample Generation
logmu = 1;
logstd = 0.3;
gaussstd = sqrt(log(1+logstd^2));
gaussmu = log(logmu) - 0.5*gaussstd^2;

option = 'SCM'; order = 3; % quadrature order in SCM
%option = 'MCS'; N=1000; % samples of random field 

switch option
    case 'MCS'
        % etag=randn(numel(lam),np,np,N); % random field coefficients for each sample
        
        % repeat the same subdomain rf over all subdomains for each sample
        etag = zeros(numel(lam),np,np,N);
        ts = randn(numel(lam),N);
        for i = 1:N
            etag(:,:,:,i) =repmat(ts(:,i),[1,np,np]);
        end

    case 'SCM'
        [ nodes, weights ] = nwspgr ('kpn', numel(lam) , order );
        N = numel(weights); % number of quadrature samples 
        % weights is: N x 1 -> vector
        % nodes is N x numel(lam) -> rectangular matrix
        etag = zeros(numel(lam),np,np,N);
        for i = 1:N 
            etag(:,:,:,i) = repmat(nodes(i,:)',[1,np,np]);
        end
    otherwise
        return
end
...
%% no correlation rf check

F2alt =nan(L/ds*np,L/ds*np,N); % random field samples 
for ii=1:np
    for jj=1:np
        tmp=phi*sqrt(diag(lam))*squeeze(etag(:,jj,ii,:)); %generations
        F2alt((jj-1)*L/ds+1:jj*L/ds,(ii-1)*L/ds+1:ii*L/ds,:)=reshape(tmp,[L/ds,L/ds,N]);
    end
end
F2alt = gaussmu + gaussstd.*F2alt;
F2alt = exp(F2alt); % transform to lognormal field
disp(1)
...
% re-adjust the Gauss random field with assigned mean and covariance 
F2 = gaussmu + gaussstd.*F2;
F2 = exp(F2); % transform to lognormal field
disp(elaps)

% % plotting random field realizations
% figure,
% for i = 1:25
%     subplot(5,5,i);
%     surf(F2(:,:,i),'EdgeColor','none'); view(0,270)
%     for ll=1:np-1
%         hold on,
%         subplot(5,5,i);
...
Eout.E = squeeze(reshape(F2(1:1/ds:end,1:1/ds:end,:),[(L/ds)*(L/ds),1,size(F2,3)]));
samples = size(Eout.E,2);
expterm = size(etag,1);

% save('segmentRFsample_SCM_q3n13_02klegrid',"expterm","samples","Eout","order")
% save('segmentRFsample_MCS_s1kn13_02klegrid',"expterm","samples","Eout")
disp(1)

%% check mean and variance
% Eout.E = squeeze(reshape(F2alt,[size(F2,1)*size(F2,2),1,size(F2,3)]));
...
switch option
    case 'MCS'
        for i = 1:N
            ...
            mualt(i) = mean(F2alt(:,:,i),'all');
            varsampalt(i) = var(F2alt(:,:,i),0,'all');
        end
    case 'SCM'
        mu = sum(Eout.E.*repmat(Eout.w',(L/ds)*(L/ds),1),2);
        varsamp = sum((Eout.E).^2.*repmat(Eout.w',(L/ds)*(L/ds),1),2)-mu.^2;
    otherwise
        return
end
